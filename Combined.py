...
from linear_rl_agents import LinearRlAgentV2,RandomAgent
...
class MCTS():
    ...
    def __init__(self,root,model,args):
        ...
        self.fast_board = FastBoard()
        self.A = LinearRlAgentV2("A",3)
        self.B = LinearRlAgentV2("B",3)
        ...
        if self.args["random"] == 1:
            ...
            if state.is_terminal():
                ...
                while not state.is_terminal():
                    ...
                    if player == "A":
                        state = self.A.action(state)
                    else:
                        state = self.B.action(state)
                    if state.is_terminal():
                        return state.reward()   
        else:
            state = node.state
...
def run_santorini(agent1 = LinearRlAgentV2("A",3), agent2 = LinearRlAgentV2("B",3)):
    '''
